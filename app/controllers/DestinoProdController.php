...
use App\Models\DestinoProd;

class DestinoProdController extends Controller
{
    private $destinoModel;

    public function __construct()
    {
        $this->destinoModel = new DestinoProd();
    ...
    public function index()
    {
        ...
        try {
            $this->view('qa/destinos/produccion', [
                'data' => [
                    'destinos' => $this->destinoModel->getDestinosProduccion($user->area_id, $user->codigo_empleado)
                ]
            ]);
        } catch (\Exception $e) {
            Logger::error('Error al cargar vista de destinos producción', [
                'error' => $e->getMessage()
            ]);
