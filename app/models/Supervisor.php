...
class Supervisor extends Model
{
    ...
    public function getProduccionDiaria($area_id, $item = '', $jtWo = '')
    {   
        $sql = "SELECT r.*, u.codigo_empleado, u.nombre AS nombre_empleado, m.nombre AS nombre_maquina 
                FROM registro r 
                JOIN users u ON r.codigo_empleado = u.codigo_empleado 
                JOIN maquinas m ON r.maquina = m.id
        ...
        if ($stmt) {
            ...
            // Inicializar arrays para mantener totales
            $totalProduccion = 0;
            $totalScrap = 0;
            ...
            while ($row = $result->fetch_assoc()) {
                ...
                // Inicializar el array para la máquina si no existe
                if (!isset($produccion_por_maquina_empleado[$maquina_id])) {
                    $produccion_por_maquina_empleado[$maquina_id] = [
                        'nombre_maquina' => $row['nombre_maquina'],
                        'empleados' => []
                    ];
                }
                
                // Inicializar el array para el empleado si no existe
                if (!isset($produccion_por_maquina_empleado[$maquina_id]['empleados'][$empleado_codigo])) {
                    $produccion_por_maquina_empleado[$maquina_id]['empleados'][$empleado_codigo] = [
                ...
                $produccion_por_maquina_empleado[$maquina_id]['empleados'][$empleado_codigo]['total_produccion'] += $row['cantidad_produccion'];
                $produccion_por_maquina_empleado[$maquina_id]['empleados'][$empleado_codigo]['total_scrap'] += $row['cantidad_scrapt'];
